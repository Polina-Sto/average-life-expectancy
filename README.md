...
for i in df.columns:
   ...
   # Заполняем пропуски средним значением по миру для каждого года
        fill = df[i].mean()
...
<img width="382" height="262" alt="image" src="https://github.com/user-attachments/assets/c5cf5a8c-cdcf-4ea5-9d86-3483b960e2b5" />
...
<img width="476" height="274" alt="image" src="https://github.com/user-attachments/assets/88b7c969-50ae-41e9-b282-ddd38e1c480b" />


### №3. Как изменилась средняя продолжительность жизни в мире с 2000 по 2015 год?
Поиск среднего значения по определённому году(т.е. по столбцу). Посчитаем и выведем на сколько изменилась средняя продолжительность жизни.
```python
x = df['2000'].mean()
print(x)
y = df['2015'].mean()
...
То есть мы постараемся найти все значения, которые сильно отличаются от остальных. Например, средняя продолжительность жизни составляет 12 лет, Такого явно мне может быть в рамках целой страны. Будем считать, что это значение не может составлять менее 27 лет в рамках одной страны. Заменим все значения в массиве менее 27. *при запуске цикла будет вывадиться график выбросов по каждому году.
```python
for i in df.columns:
    if i in ['Country Name', 'Country Code', 
               'Indicator Name', 'Indicator Code', 'Unnamed: 69']:
        ...
        sns.boxplot(data=df, x=i) # при построении графика выбросов, будут построены графики по каждому году DF
        plt.show()
```
...
```
Выберем данные только за определённый год. В нашем случае 2023. Сделаем объёдинение таблиц.
```python
year = '2023'
...
Для более корректного отображения данных заполним пустые значения на медианные в итоговой таблице:
```python
...
<img width="408" height="280" alt="image" src="https://github.com/user-attachments/assets/324d7dc0-99da-489c-9cc4-5a33c96ebfce" />
 />  <br />
Как можно увидеть на графике, показатель коррелирует с ВВП на душу населения на 64% - это является средним показателем корреляции. Однако это не означает, что уровень ВВП напрямую увеличивает долголетие — возможны скрытые факторы (например, уровень дохода, доступ к медицине). Останьные значения не демонстрируют высокий показатель корреляции сос редней продолжительностью жизни.

Корреляция по 2023 году: <br />
<img width="408" height="280" alt="image" src="https://github.com/user-attachments/assets/5abcb3ae-17ee-40a6-8f10-8a6c7bdc57c3" />
 /> <br />
Однако в 2023 году происходит немного другое. Здесь также можно наблюдать корреляцию между продолжительностью жизни ВВП - 61%. Также за 2023 год данные показывают, незначительную корреляцию других признаков. ч /> <br />

## Шаг 6. Создание простой модели регрессионнного анализа
Данная модель имеет ограничения, поскольку наша итоговая таблица содержит данный только по 1 году, а также есть много пропусков, которые были заполнены медианными значениями. При этом она использует только новые значения, не беря в расчёт данные за 1960 и тп., которые сейчас не является в полной мере актуальным.
```python
...
Попробуем предсказать уровень продолжительности жизни при ВВП на душу населения(в долларах США) 10000, уровнем образования 50(используется 100 бальная шкала; например, в Германии эта оценка составляет 0,94 за 2020 год, а в РФ 0,82 за 2020 год) и уровнем инвестиций в здравоохранение(процент инвестиций в здравоохранение от уровня ВВП страны) равным 4.
```python
years = model.predict([[30000, 4, 50]])
print(years)
```
Получим следующий результат: /> <br />
[63.74654083]

## Выводы
1. **Продолжительность жизни в мире растёт**
   С 2000 по 2015 год средняя продолжительность жизни увеличилась на ~4.6 лет, что свидетельствует о глобальном улучшении качества жизни.

2. **Главные факторы, влияющие на долголетие**  
   Анализ корреляций показал:
   - Значительное влияние ВВП на душу населения (r ≈ 0.62) — экономическое развитие играет важную роль.
   - Уровень образование и инвестиций в здравоохранение влиет незначительно.
